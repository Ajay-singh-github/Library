...
export default function Stuffadd(props){
    //  const classes = useStyles()
     ...
     const [date,setDate]=useState();
       ...
       const validation=()=>{
        var submitRecord=true
         ...
         if(!preparation)
         {
            handleError(true,'preparation','Pls Input salary')
            submitRecord=false
         }

         if(!date)
         {
            handleError(true,'date','Pls Select date of Joing')
         }
     ...
     const handleIcon=(event)=>{
      ...
      handleError(true,'icon','')
       ...
       formData.append('salary',preparation)
       formData.append("date",date)
       formData.append('picture',CategoryIcon.bytes)   
       var result=await postData('login/student_add_subm',formData)
       if(result.status)
             ...
             <div style={{margin:50}}>
                <Grid container spacing={2}>
                   <Grid item xs={12} style={{marginBottom:20}}>
                       <Heading title='Stuff Information' picture={"stuff.png"} />
                   </Grid>
                   <Grid item xs={6}>
                   ...
                   <Grid item xs={6}>
                       <TextField placeholder='Salary' label="Salary" fullWidth  onChange={(event)=>setPreparation(event.target.value)}
                   ...
                   <Grid item xs={12}>
                    <text style={{fontWeight:"bolder"}}>Date Of Joing: </text><input type="date" onChange={(event)=>setDate(event.target.value)} />
                   </Grid>
