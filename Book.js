    ...
    //  const classes = useStyles()
     ...
     const [preparation,setPreparation]=useState()
       ...
       const validation=()=>{
        var submitRecord=true
         if(!name)
         {
            handleError(true,'name','Pls Input Name of Book')
            submitRecord=false
         }
         if(!mobile)
         {
            handleError(true,'mobile','Pls Input Name Of Relative Book Like Competition')
            submitRecord=false
         }
     ...
     const handleIcon=(event)=>{
      ...
      handleError(true,'icon','')
       ...
       formData.append('relative',mobile)  
      
    
       formData.append('picture',CategoryIcon.bytes)   
      var result=await postData('login/student_add',formData)
       if(result.status)
             ...
             <div style={{margin:50}}>
                <Grid container spacing={2}>
                   <Grid item xs={12} style={{marginBottom:20}}>
                       <Heading title='Book Information' picture={"book.webp"} />
                   </Grid>
                   <Grid item xs={6}>
                       <TextField placeholder='Book Name' label="Book Name" fullWidth onChange={(event)=>setName(event.target.value)} 
                        error={resError?.name?.error}
                        helperText={resError?.name?.message}
                        onFocus={()=>handleError(false,'name','')}/>
                   </Grid>
                   <Grid item xs={6}>
                       <TextField fullWidth label="Relative"
